...
Channel::Channel(const std::string& name) 
    : Channelname(name), 
      limit(0), 
      topicRestricted(true),    // +t by default
      inviteOnly(false),         // -i by default
      hasPassword(false),        // -k by default
      hasLimit(false),           // -l by default
      currentUserCount(0) {}
...
bool Channel::hasPasswordSet() const { return hasPassword; }
void Channel::setHasPassword(bool value) { hasPassword = value; }

bool Channel::hasUserLimit() const { return hasLimit; }
void Channel::setHasLimit(bool value) { hasLimit = value; }

int Channel::getCurrentUserCount() const { return currentUserCount; }
void Channel::incrementUserCount() { currentUserCount++; }
void Channel::decrementUserCount() { if (currentUserCount > 0) currentUserCount--; }
void Channel::resetUserCount() { currentUserCount = 0; }
...
void Server::createChannel(const std::string& channelName, const std::string& nickname, int fd) {
    ...
    if (it == channels.end()) {
        ...
        // User count is automatically tracked by addClient()
